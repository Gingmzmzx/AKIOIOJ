...
async def render_or_json_problem_list(self, page, ppcount, pcount, pdocs,
                                      category, psdict, domain_id, **kwargs):
  if builtin.BLESSING and domain_id and psdict:
    psdict['blessing_msg'] = builtin.BLESSING_MESSAGE
    psdict['blessing'] = builtin.BLESSING
...
class ProblemCategoryHandler(base.OperationHandler):
  ...
  async def get(self, *, category: str, page: int=1):
    ...
    await render_or_json_problem_list(self, page=page, ppcount=ppcount, pcount=pcount,
                                      ...
                                      page_title=page_title, path_components=path_components, domain_id=self.domain_id)
...
class ProblemDetailHandler(base.OperationHandler):
  ...
  async def get(self, *, pid: document.convert_doc_id):
    ...
    # Set AC Message to "AK NOIP 2023!!!"
    if builtin.BLESSING and self.domain_id == "system":
      pdoc['ac_msg'] = builtin.BLESSING_MESSAGE
